...
(function () {
  function getSlugFromUrl() {
    ...
    // getFullSlugFromUrl() returns window.location.pathname as-is, which the
    // browser keeps percent-encoded. contentIndex keys are plain text, so for
    // non-ASCII slugs (e.g. Cyrillic) nothing ever matches and the local graph
    // degenerates into a single orphan node labelled with the raw URL.
    try {
  ...
  function initGraph() {
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      var hideTags = config.hideTags || [];
      var hideFolderPages = config.hideFolderPages;
      var showTags = config.showTags;
      var focusOnHover = config.focusOnHover;
      var enableRadial = config.enableRadial;

      var data;
      // Generated pages must be recognised from the RAW contentIndex keys:
      // simplifySlug() trims the "index" suffix, so "npc/index" collapses to
      // "npc" and becomes indistinguishable from a real note afterwards.
      var tagPageSlugs = new Set();
      var folderPageSlugs = new Set();
      try {
        ...
        for (var key in dataRaw) {
          ...
          if (key === "tags" || key.indexOf("tags/") === 0) {
            tagPageSlugs.add(simple);
          } else if (key.length > 6 && key.slice(-6) === "/index") {
            folderPageSlugs.add(simple);
          }
        }
      } catch (err) {
      ...
      // Notes carrying any `hideTags` tag are dropped from the graph entirely.
      // Removing them from validLinks also drops every link pointing at them,
      // so no dangling edges remain. They stay in contentIndex, so search,
      // RSS and sitemap are unaffected.
      var hiddenNodes = new Set();
      if (hideTags.length > 0) {
        data.forEach(function (details, key) {
          var nodeTags = details.tags || [];
          for (var i = 0; i < nodeTags.length; i++) {
            if (hideTags.indexOf(nodeTags[i]) !== -1) {
              hiddenNodes.add(key);
              break;
            }
          }
      ...
      // Tag pages and folder indexes are generated pages that live in
      // contentIndex alongside real notes. When showTags is on, a tag page
      // shares its slug with the synthetic tag node and picks up its links —
      // but with showTags off nothing links to it, so it drifts away as an
      // isolated node. Folder indexes have no inbound links either way.
      if (!showTags) {
        tagPageSlugs.forEach(function (key) {
          hiddenNodes.add(key);
        });
      }
      if (hideFolderPages) {
        folderPageSlugs.forEach(function (key) {
          hiddenNodes.add(key);
        });
      }

      if (hiddenNodes.size > 0) {
        hiddenNodes.forEach(function (key) {
          validLinks.delete(key);
        });
      }

      data.forEach(function (details, source) {
        if (hiddenNodes.has(source)) return;
